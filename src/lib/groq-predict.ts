...
export async function generateFallbackPrediction(
  match: MatchData
  ...
  const homeForm = getFormFromMatches(homeMatches, homeTeamId)
  const awayForm = getFormFromMatches(awayMatches, awayTeamId)

  const homeFormScore = homeForm.weightedScore
  const awayFormScore = awayForm.weightedScore
  const total = homeFormScore + awayFormScore
  const baseHomeProb = total > 0 ? Math.round((homeFormScore / total) * 70 + 15) : 50
  const baseAwayProb = total > 0 ? Math.round((awayFormScore / total) * 70 + 15) : 50
  const drawProb = 100 - baseHomeProb - baseAwayProb

  const homeWinProb = Math.max(5, Math.min(85, baseHomeProb))
  const awayWinProb = Math.max(5, Math.min(85, baseAwayProb))
  const draw = 100 - homeWinProb - awayWinProb

  const favorite = homeWinProb > awayWinProb ? 'home' : 'away'
  const predictedHome = favorite === 'home' ? 2 : 1
  const predictedAway = favorite === 'away' ? 2 : 1
  ...
  return {
    ...
    keyFactors: [
      { factor: 'Recent form analysis', favors: favorite, impact: 'MEDIUM' },
    ],
    reasoning: `Based on recent form analysis, ${
      match.homeTeam.name
    } has a form score of ${homeFormScore}/15 while ${
      match.awayTeam.name
    } has ${awayFormScore}/15. ${
      homeWinProb > awayWinProb
        ? match.homeTeam.name + ' enters as the favorite.'
        : match.awayTeam.name + ' enters as the favorite.'
    }`,
    formAnalysis: {
      homeFormRating: Math.round((homeFormScore / 15) * 10),
      awayFormRating: Math.round((awayFormScore / 15) * 10),
      comment: 'Based on recent match performance data.',
    },
    riskFactors: ['Incomplete data - some stats may be unavailable'],
    recommendedBet: `${favorite === 'home' ? match.homeTeam.name : match.awayTeam.name} to win`,
    predictionVersion: 'pre-match',
    liveContext: null,
  }
}
